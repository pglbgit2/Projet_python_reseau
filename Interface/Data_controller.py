...
Path_join_page = "/View/Sprites/Join_page/"
...
go_to_join_game = pg.USEREVENT + 108
...
class Button:
    ...
    def get_width(self):
        return self.__image.get_size()[0]

    def get_height(self):
        return self.__image.get_size()[1]
    ...
    def set_image(self):
        self.__image = pg.transform.scale(self.__image, (self.get_width(), self.get_height()))
...
class Texte_button(Button):
    ...
    def overtext(self, pos):
        return self.get_enable() and self.get_left() <= pos[
            0] <= self.get_left() + self.get_width() and self.get_up() <= pos[1] <= self.get_up() + self.get_height()
...
HP_join_game = Button(Pos_HP_join_game, Path_HP_join_game, go_to_join_game)
HP_join_game.resize((window_width, winddow_height))
...
SP_validate_txt_R.topleft = (
    Pos_SP_validate[0] - SP_validate_txt.get_width() / 2, Pos_SP_validate[1] - SP_validate_txt.get_height() / 2)

# Join Page Ones :

JP_back = Button(Pos_JP_back, Path_JP_back, None)
JP_back.resize((window_width, winddow_height))

JP_support = Button(Pos_JP_support, Path_JP_support, None)
JP_support.resize((window_width, winddow_height))
...
SP_go_home_txt_R.topleft = (
    Pos_SP_go_home[0] - SP_go_home_txt.get_width() / 2, Pos_SP_go_home[1] - SP_go_home_txt.get_height() / 2)
# SP_validate.set_size(( window_width /10, winddow_height /10 ))
...
def set_screen_join_page(screen):
    disable_all_HP_button()
    enable_all_JP_button()
    JP_back.draw_image_center(screen)
...
def set_screen_SP(screen):
    ...
    screen.blit(SP_validate_txt, (
        Pos_SP_validate[0] - SP_validate_txt.get_width() / 2, Pos_SP_validate[1] - SP_validate_txt.get_height() / 2))
    screen.blit(SP_go_home_txt, (
        Pos_SP_go_home[0] - SP_go_home_txt.get_width() / 2, Pos_SP_go_home[1] - SP_go_home_txt.get_height() / 2))
...
#                 Cur_page = "Home"
#                 HP_tittle.set_disable()
>>>>>>> 6804081c76ee0744e9321e9c71a9d94b5cf8f56e
